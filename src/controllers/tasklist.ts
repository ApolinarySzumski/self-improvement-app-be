...
export const createTask = async (req: Request, res: Response) => {
    try {
        ...
        const newTask = await pool.query(
            "INSERT INTO tasklist (task, created_at) VALUES($1,$2) RETURNING *", [task, createdAt]
        );

        res.status(201).json(newTask.rows[0]);
    } catch (e) {
        res.status(500).json({ message: "Internal Server Error", error: errorHandler(e) });
    }
};

export const getTasks = async (_: unknown, res: Response) => {
    try {
        const fetchTasksFromDB: Task[] = (await pool.query("SELECT * FROM tasklist ORDER BY tasklist_id")).rows;
...
export const getTask = async (req: Request, res: Response) => {
    try {
        const { id } = req.params
        const task = await pool.query("SELECT * FROM tasklist WHERE tasklist_id = $1", [id]);
...
export const updateTask = async (req: Request, res: Response) => {
    try {
        ...
        const taskToUpdate = await pool.query("SELECT * FROM tasklist WHERE tasklist_id = $1", [id]);
        ...
        await pool.query("UPDATE tasklist SET task = $1, updated_at = $2 WHERE tasklist_id = $3", [task, createdAt, id]);
